...
export const useLoadDashboardData = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    if (stakingDataErr || !stakingData) {
      notifications.error({ message: messages.FAILED_TO_LOAD_CHAIN_DATA, errorOrMessage: stakingDataErr! });
      return;
    }

    const [allowanceErr, allowance] = await go(api3Token.allowance(userAccount, api3Pool.address));
    if (allowanceErr || !allowance) {
      notifications.error({ message: messages.FAILED_TO_LOAD_CHAIN_DATA, errorOrMessage: allowanceErr! });
      return;
    }
